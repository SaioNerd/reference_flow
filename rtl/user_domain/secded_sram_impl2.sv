  ...
  generate
    if (SECDEDBypass) begin : gen_bypass
      ...
      for (genvar p = 0; p < NumPorts; p++) begin : gen_ports
        ...
        secded_data_t secded_wdata_generated; // Generated by encoder, used for repair buffer
        ...
        for (genvar b = 0; b < BeWidth; b++) begin : gen_bytes
          ...
          // ====================================================================
          // Re-encode the newly corrected read data (dec_out) for write-back
          // ====================================================================
          logic [12:0] repair_enc_out;
          secded_byte_encode i_repair_encode (
            .data_in     (dec_out),        // Takes the fixed data from the decoder
            .encoded_out (repair_enc_out)
          );

          // FIX: Assign the re-encoded data to the generated wdata array
          assign secded_wdata_generated[b*16 +: 16] = {3'b000, repair_enc_out};
